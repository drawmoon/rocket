...
从实现方式上看，Rerank 可以分为两大类：一类是无模型的排序融合算法（如 RRF、Weighted Ranker），通过数学方法将多个检索器的结果合并为统一排序，延迟极低、无需训练；另一类是基于模型的精排方法（如 BGE Rerank、Cohere Rerank），利用 Cross-Encoder 对查询和文档进行逐对语义理解打分，排序质量更高但计算成本相对较大。在实际项目中，通常先用 RRF 或加权融合对多路召回结果做粗排融合，再用 Cross-Encoder 做 Top-K 精排，形成完整的检索到重排序链路。
...
RRF 是一种 无模型、无训练的排序融合算法，核心思想是融合后按综合得分排序，交叉出现的更高一致性者胜出。
...
保持数据的唯一性与内存效率，通常建议把文本转换成哈希。下面是转换成 MD5 哈希的代码实现：
...
#### Weighted RRF (Weighted Reciprocal Rank Fusion)
...
```python
def weighted_rrf(
    document_lists: list[list[str]], weights: list[float] = None, k: int = 60
...
**线性加权融合对比 RRF 与 Weighted RRF**

- 如果使用多个不同的 Embeddings 模型，线性加权运用简单且非常有效。
- 如果混合了 Embeddings + BM25，则必须实现归一化，向量分值通常在 $0 \sim 1$ 之间，而 BM25 分数可能在 $0 \sim 100$，高分值的通道会完全淹没低分值的通道。如果分数的分布非常稳定且归一化得当，它能比 RRF 更精准地反映文档的相关度。
...
- 支持 RRF（Reciprocal Rank Fusion）与加权 RRF 两种融合策略
- 兼容任意 LangChain Retriever 接口，包括向量检索、关键词检索、图检索等
- 可横向扩展，任意数量子检索器均可组合
...
RRF 模式仅使用各检索器的排序名次进行融合，无需归一化原始分数。
...
加权 RRF 通过 `weights` 参数为每个检索器分配不同的权重系数，从而在融合时放大高置信度检索器的影响。

```python
from langchain_classic.retrievers.ensemble import EnsembleRetriever

# 假设 dense 向量检索对当前业务更可靠，给予更高权重
ensemble_weighted = EnsembleRetriever(
    retrievers=[dense_retriever, sparse_retriever],
    weights=[0.7, 0.3],  # dense 占 70%，BM25 占 30%
)

# 检索并对比加权前后的结果差异
docs_weighted = ensemble_weighted.invoke("什么是机器学习？")
for i, doc in enumerate(docs_weighted, 1):
    print(f"Rank {i}: {doc.page_content[:80]}...")
```

**手动实现 vs EnsembleRetriever**

|维度|手动实现（前文）|EnsembleRetriever|
|---|---|---|
|灵活性|高，可完全自定义逻辑|中，封装在框架内|
|开发成本|低（算法简单）| 极低（一行代码组合）|
|与 LangChain 生态集成|需自行适配 | 原生支持，可直接链入 LCEL|
|子检索器类型|任意列表|任意 LangChain Retriever|
|生产适用性|适合轻量/定制场景|适合已有 LangChain 技术栈的项目|

> 如果你已经在使用 LangChain 或 Elasticsearch 构建 RAG 流程，EnsembleRetriever 是快速实现混合检索融合的最佳实践；如果需要对融合逻辑做深度定制，仍可回退到前文的手动实现。
...
安装完成后，首先需要初始化 Cohere 客户端。建议将 API Key 存放在环境变量中，避免硬编码到代码里。
...
首先，我们需要安装 PydanticAI 依赖:

```sh
pip install pydantic-ai
```

下面是一个最干净、最简单的 PydanticAI Agent 实现代码，可让 Agent 具备基础问答能力：
...
Agent 的核心价值在于能够调用外部工具。下面演示如何给 Agent 添加一个搜索工具，让它能够"查资料"后再回答问题：
...
def search_knowledgebase(query: str) -> str:
    """模拟知识库搜索"""
    return f"搜索结果：{query} 是检索增强生成技术。"


agent = Agent(
    model="openai:gpt-4o",
    system_prompt="你先使用工具搜索信息，然后基于搜索结果回答问题。",
    tools=[Tool(search_knowledgebase)],
)

result = agent.run_sync("什么是 RAG？")
print(result.data)
```

在这个示例中，Agent 接收到问题后会先调用 `search_knowledgebase` 获取信息，再基于搜索结果生成回答。这就是 Agent 与普通 Chatbot 的本质区别：**不是直接回答，而是先获取信息、再行动**。
-->
